...
import com.kakao.linknamu._core.TestConfig;
...
public class WorkspaceJpaRepositoryTest {
    ...
    @BeforeEach
    void setup() {
        User user = User.builder().email("rjsdnxogh55@gmail.com").provider(Provider.PROVIDER_GOOGLE).role(Role.ROLE_USER).password("testpassword").build();
    ...
    void findByUserIdAndWorkspaceNameTest() {
        ...
        Workspace workspace = Workspace.builder().workspaceName("워크스페이스").user(user).linkProvider(LinkProvider.NORMAL).build();
        workspaceJPARepository.save(workspace);

        Long findUserId = user.getUserId();
        String findWorkspaceName = "워크스페이스";
        // when
        Workspace findWorkspace = workspaceJPARepository.findByUserIdAndWorkspaceName(findUserId, findWorkspaceName).orElseThrow();
    ...
    void workspaceCanTakeCategoryList() {
        // given
        User user = userJPARepository.findByEmail("rjsdnxogh55@gmail.com").orElseThrow();
        Workspace workspace1 = Workspace.builder().workspaceName("워크스페이스1").user(user).linkProvider(LinkProvider.NORMAL).build();
        Workspace workspace2 = Workspace.builder().workspaceName("워크스페이스2").user(user).linkProvider(LinkProvider.NORMAL).build();
